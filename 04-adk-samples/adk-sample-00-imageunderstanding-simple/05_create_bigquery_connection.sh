...
export SERVICE_NAME="bq-image-audit-agent"
...
BUCKET_NAME=$(echo "${STAGING_BUCKET_4_OBJECT_TABLE}" | cut -d'/' -f1)
echo $BUCKET_NAME
gsutil iam ch serviceAccount:${CONNECTION_SA}:roles/storage.objectViewer gs://${BUCKET_NAME}