...
for i in range(number_of_ideas):
    creative_agent = LlmAgent(name=f"Idea{i}", 
    model = MODEL, 
...
synthesizer = LlmAgent(
    model = MODEL,
    name="Synthesizer",
    instruction="Combine results and order them by their brilliance. Remove duplicates. Score every from 1 (nothing new) to 10 - genius"
)
